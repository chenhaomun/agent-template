...
# For each non-"github-adapted" skill in skills-lock.json, diff its local copy
# against upstream. local = entry's "localPath" if present, else .agents/<skillPath>:
#   diff -u <local>  "$TMP/<repo>/<skillPath>"
```
